...
c.push(H2('What we would actually buy, once the gates clear'));
c.push(PR([{ text: 'If the diligence below clears, the asset has a precise name: the ' }, { text: 'Verified Net Cash Receivable', bold: true }, { text: ' — every monetary right actually payable to the generator under a signed Termo, after the CCEAR-D/CER offset, prior payments, penalties, taxes and transaction costs are deducted. Not the R$3.3bn headline. Not the generator’s own estimate. Only the number that survives the full waterfall.' }]));
c.push(PR([{ text: 'The right structure for that, if it becomes buyable, is a true sale of the receivable rather than a loan against the generator — cleaner for us, and it keeps a repeat programme out of the regulatory territory that catches a business of habitually lending against Brazilian receivables. Held in a dedicated vehicle, not on the operating company’s balance sheet.' }]));
c.push(PR([{ text: 'And the timing should be staged, not committed all at once: lock exclusivity now with a conditional forward-purchase agreement — the right and obligation to buy if defined conditions are met, at a pre-agreed pricing formula — so the deal flow is ours without deploying a real against an asset that does not yet exist in a clean, fixed form. Fund the bulk of it only once the Termo is signed, the volumes are known, and the two gates below have actually cleared. That gets GC into the room this week without betting capital on an unproven mechanism.' }]));

c.push(H2('The two gates, and why they are harder than a lockbox agreement'));
c.push(PR([{ text: 'A reasonable objection to all of the above is that Brazilian law generally permits assigning a credit, and that CCEE’s same-CNPJ requirement can be solved with a blocked account in the generator’s name, swept automatically to us. That is the right first instinct. It is not obviously sufficient here, for two reasons specific to this instrument rather than to receivables assignment generally.' }]));
c.push(N('This is not a discrete invoice — it is a running netting entry. Article 6º §§3–5 does not have CCEE cut a cheque. It suspends the generator’s other CCEAR-D and CER settlement obligations, then resumes billing net of the reduction, inside the agent’s continuous multilateral settlement relationship with CCEE. A lockbox is the right tool for redirecting a fixed payable. It is not obviously the right tool for carving one line out of an agent’s ongoing settlement account and rerouting it to a party with no CCEE agent registration — that is closer to asking CCEE to change how its settlement system treats a specific credit than to signing an account-control agreement with a bank. CCEE’s own adapted commercialisation rules for this scheme are not even finalised until September 2026.'));
c.push(N('The Termo binds the generator personally, for the life of the claim, and breach suspends the money. Subcláusula Oitava: a five-day cure notice, and if it is not cured, compensation is suspended while every other obligation survives. The generator must file a court petition renouncing litigation within ten days of signing, accept CCEE’s classification irrevogável e incondicional before the cash amount is even known, and never oppose a provisional court order against the União or CCEE. A buyer of this receivable is not only underwriting Treasury and CCEE timing risk — it is underwriting eighteen months of the ORIGINAL generator’s compliance behaviour, where a single missed filing kills the asset. That is a different and larger risk than a discount rate captures, and it does not disappear once the account-control paperwork is signed.'));
...
c.push(callout('CANONICAL VERSION — use this wording with GC and with any capital partner', [
  ...
  'A separate vehicle locks exclusivity now through conditional forward-purchase agreements, and deploys real capital only against the Verified Net Cash Receivable — the positive amount surviving the full waterfall — once a Termo is signed, title is clean, and payment control is enforceable against CCEE’s netting-based settlement, not assumed from a generic lockbox. Gross compensation and accounting liability relief are never treated as purchasable face value.',
  'Mandate first. Fees first. Lock exclusivity early. Verify the cash and the payment mechanics. Deploy principal last, and only once both have cleared.'
]));
