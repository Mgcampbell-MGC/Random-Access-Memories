...
c.push(H2('The gate that decides whether the capital line exists'));
c.push(PR([{ text: 'CCEE’s own banking rules require the settlement account to carry the same CNPJ as the market agent. That is a strong indication the settlement rails are agent-centric and will not simply redirect payment to a third-party buyer. Combined with the Portaria’s total silence on assignment — zero occurrences of cessão, transferência, oneração, penhor or caução — this is not a mechanical detail. It decides whether we can ever hold the asset.' }]));
c.push(PR([{ text: 'The rule is: no enforceable payment control, no principal. If counsel concludes CCEE will not recognise an assignee and no adequate account-control structure can be built, we still run the advisory business — we simply never fund. That outcome costs us nothing we have already spent.', bold: true }]));
...
c.push(callout('CANONICAL VERSION — use this wording with GC and with any capital partner', [
  ...
  'A separate vehicle finances only the positive net cash remaining after the full waterfall, in stages, and only once title, liens and enforceable payment control are settled. Gross compensation and accounting liability relief are never treated as purchasable face value.',
  'Mandate first. Fees first. Verification second. Third-party capital third. Our own principal last.'
]));
